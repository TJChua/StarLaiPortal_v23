                    ...
                    if (Priority != null)
                    {
                        if (Priority.PriorityName == "Urgent")
                        {
                            if (PostingDate.TimeOfDay <= TimeSpan.ParseExact(Transporter.U_CutOffTime, "hhmm", CultureInfo.InvariantCulture))
                            {
                                ...
                                {
                                    int finaldelivery = 0;
                                    for (DateTime date = PostingDate.Date; date <= DeliveryDate.Date.AddDays(sundaycnt); date = date.AddDays(1))
                                    {
                                        if (date.DayOfWeek == DayOfWeek.Sunday)
                                        {
                                            finaldelivery++;
                                    ...
                                    DeliveryDate = DeliveryDate.AddDays(finaldelivery);
                                }

                                if (DeliveryDate.DayOfWeek.ToString() == "Sunday")
                                {
                                    DeliveryDate = DeliveryDate.Date.AddDays(1);
                                    DeliveryDate = DeliveryDate.Date.Add(TimeSpan.ParseExact(Transporter.U_UrgentTime, "hhmm", CultureInfo.InvariantCulture));
                                }
                            }
                            else
                            {
                        ...
                        else
                        {
                            if (PostingDate.TimeOfDay <= TimeSpan.ParseExact(Transporter.U_CutOffTime, "hhmm", CultureInfo.InvariantCulture))
                            {
                                ...
                                {
                                    DeliveryDate = DeliveryDate.Date.AddDays(1);
                                    DeliveryDate = DeliveryDate.Date.Add(TimeSpan.ParseExact(Transporter.U_NormalTime, "hhmm", CultureInfo.InvariantCulture));
                                }
                            }
                            else
                            {
